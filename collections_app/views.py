...
def serve_unity_static(request, file_path):
    """Custom static file serving for Unity WebGL files with proper MIME types"""
    import os
    from django.http import HttpResponse, Http404
    
    # Build the full file path
    static_root = getattr(settings, 'STATIC_ROOT', os.path.join(settings.BASE_DIR, 'staticfiles'))
    full_path = os.path.join(static_root, 'web_build', file_path)
    
    if not os.path.exists(full_path):
        # Also try from STATICFILES_DIRS
        for static_dir in getattr(settings, 'STATICFILES_DIRS', []):
            alt_path = os.path.join(static_dir, 'web_build', file_path)
            if os.path.exists(alt_path):
                full_path = alt_path
                break
        else:
            raise Http404(f"Unity static file not found: {file_path}")
